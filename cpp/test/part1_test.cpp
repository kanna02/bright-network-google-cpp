...
TEST(Part1, playVideo) {
  ...
  videoPlayer.stopVideo(); //added
  std::string output = testing::internal::GetCapturedStdout();
  std::vector<std::string> commandOutput = splitlines(output);
  ASSERT_EQ(commandOutput.size(), 2); //was 1
  EXPECT_THAT(commandOutput[0], HasSubstr("Playing video: Amazing Cats"));
  EXPECT_THAT(commandOutput[1], HasSubstr("Stopping video: Amazing Cats")); //added
...
TEST(Part1, playVideoStopPrevious) {
  ...
  std::string output = testing::internal::GetCapturedStdout();
  std::vector<std::string> commandOutput = splitlines(output);
  ASSERT_EQ(commandOutput.size(), 4); // was 3
  EXPECT_THAT(commandOutput[0], HasSubstr("Playing video: Amazing Cats"));
  ...
  EXPECT_THAT(commandOutput[3], HasSubstr("Stopping video: Funny Dogs")); //added
...
TEST(Part1, playVideoDontStopPreviousIfNonExistent) {
  ...
  std::string output = testing::internal::GetCapturedStdout();
  std::vector<std::string> commandOutput = splitlines(output);
  ASSERT_EQ(commandOutput.size(), 3); // was 2
  EXPECT_THAT(commandOutput[0], HasSubstr("Playing video: Amazing Cats")); 
...
TEST(Part1, playRandomVideo) {
  ...
  EXPECT_THAT( // added
      commandOutput[1],
      ContainsRegex("Stopping video: (Amazing Cats|Another Cat Video|Funny "
                    "Dogs|Life at Google|Video about nothing)"));
}
...
TEST(Part1, showPlaying) {
  ...
  EXPECT_THAT(commandOutput[2], HasSubstr("Stopping video: Amazing Cats")); // added
...
TEST(Part1, pauseVideoShowVideo) {
  ...
  std::string output = testing::internal::GetCapturedStdout();
  std::vector<std::string> commandOutput = splitlines(output);
  ASSERT_EQ(commandOutput.size(), 5); // was 3
  EXPECT_THAT(commandOutput[0], HasSubstr("Stopping video: Amazing Cats")); //added
  EXPECT_THAT(commandOutput[1], HasSubstr("Playing video: Amazing Cats")); //0
  EXPECT_THAT(commandOutput[2], HasSubstr("Pausing video: Amazing Cats")); //1
  EXPECT_THAT(commandOutput[3], //2
              HasSubstr("Currently playing: Amazing Cats "
                        "(amazing_cats_video_id) [#cat #animal] - PAUSED"));
  EXPECT_THAT(commandOutput[4], HasSubstr("Stopping video: Amazing Cats")); //added

}

// forgot the original test case
TEST(Part1, pauseVideoPlayVideo) {
  ...
  std::string output = testing::internal::GetCapturedStdout();
  std::vector<std::string> commandOutput = splitlines(output);
  ASSERT_EQ(commandOutput.size(), 6); // was 5?
  EXPECT_THAT(commandOutput[0], HasSubstr("Playing video: Amazing Cats")); // 0
  EXPECT_THAT(commandOutput[1], HasSubstr("Pausing video: Amazing Cats")); //1
  EXPECT_THAT(commandOutput[2], HasSubstr("Stopping video: Amazing Cats")); //2
  EXPECT_THAT(commandOutput[3], HasSubstr("Playing video: Amazing Cats")); //3
  EXPECT_THAT(commandOutput[4], //4
              HasSubstr("Currently playing: Amazing Cats "
                        "(amazing_cats_video_id) [#cat #animal]"));
  EXPECT_THAT(commandOutput[5], HasSubstr("Stopping video: Amazing Cats")); //2
...
TEST(Part1, pauseAlreadyPausedVideo) {
  ...
  EXPECT_THAT(commandOutput[3], HasSubstr("Stopping video: Amazing Cats"));
...
TEST(Part1, pauseVideoNothingPlaying) {
  ...
  std::vector<std::string> commandOutput = splitlines(output);
  ASSERT_EQ(commandOutput.size(), 1); 
  EXPECT_THAT(commandOutput[0], HasSubstr("Cannot pause video: No video is currently playing")); 
}
...
TEST(Part1, continueVideo) {
  ...
  EXPECT_THAT(commandOutput[3], HasSubstr("Stopping video: Amazing Cats")); //added
...
TEST(Part1, continueVideoNotPaused) {
  ...
  std::string output = testing::internal::GetCapturedStdout();
  std::vector<std::string> commandOutput = splitlines(output);
  ASSERT_EQ(commandOutput.size(), 3);  // was 1
  EXPECT_THAT(commandOutput[0], HasSubstr("Playing video: Amazing Cats")); //added
...
TEST(Part1, continueVideoNothingPlaying) {
  ...
  ASSERT_EQ(commandOutput.size(), 1);
  EXPECT_THAT(
      commandOutput[0], HasSubstr("Cannot continue video: No video is currently playing"));
}
